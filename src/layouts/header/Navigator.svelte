...
// Make these reactive to locale changes
const t = $derived(i18nit(locale));

// Define home route and navigation routes configuration - made reactive
const homeRoute = $derived(getRelativeLocaleUrl(locale));
const routes = $derived<{ path: string; extra?: string[]; icon: `${string}--${string}`; label: string }[]>([
	...
	{ label: t("navigation.instruction"), path: getRelativeLocaleUrl(locale, "/instruction"), icon: "lucide--book" },
...
<nav bind:this={navigator} class:translate-x-full={!menu} class:rtl:-translate-x-full={!menu} class="fixed top-0 end-0 flex flex-col justify-between items-start gap-5 p-5 bg-background h-full sm:contents overflow-hidden transition-transform">
	<header class="grid gap-5 text-secondary grid-rows-[repeat(5,1fr)] sm:grid-rows-none sm:grid-cols-[repeat(5,1fr)]">
